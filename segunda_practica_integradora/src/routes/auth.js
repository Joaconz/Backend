...
router.post('/login',passport.authenticate('login', {failureRedirect: '/auth/faillogin'}) , auth('session'), async (req, res)=> {

    if (req.user === undefined) {
        res.status(401).send({status: 'error', message: 'El usuario no existe'})
    }
    req.session.user = {
